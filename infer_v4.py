...
def accept_plate_box(x1, y1, x2, y2, W, H, conf):
    bw = max(1.0, x2 - x1)
    bh = max(1.0, y2 - y1)
    ar = bw / bh
    area = bw * bh
    ...
    return not (
        bw < max(55, W * 0.02)
        or bh < max(22, H * 0.015)
        or ar < 1.2
        or ar > 8
        or area_frac < 0.00010
        or area_frac > 0.14
    )
...
def main():
    ...
    for img_path in iter_images(input_path):
        ...
        if boxes is not None and len(boxes) > 0:
            for i, (xy, c) in enumerate(zip(boxes.xyxy.cpu().numpy(), boxes.conf.cpu().numpy())):
                x1, y1, x2, y2 = map(float, xy)
                xi1, yi1, xi2, yi2 = pad_xyxy(x1, y1, x2, y2, W, H, args.pad)

                if not accept_plate_box(xi1, yi1, xi2, yi2, W, H, c):
                    continue

                cv2.rectangle(overlay, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
                cv2.putText(overlay, f"{c:.3f}", (int(x1), max(0, int(y1) - 6)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                ...
                item = {
                    ...
                    "plate_conf": c,
                    "plate_box_xyxy": [xi1, yi1, xi2, yi2],
